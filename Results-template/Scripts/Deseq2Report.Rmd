...
system(paste0("cat temp_up.txt | grep -v \"^NA\" | /data/CCBR_Pipeliner/db/PipeDB/bin/L2P/m2h | grep -v XXXX | cut -f2 -d\" \"  >deseq2_gene_up_path",extension))
system(paste0("cat temp_dw.txt | grep -v \"^NA\" | /data/CCBR_Pipeliner/db/PipeDB/bin/L2P/m2h | grep -v XXXX | cut -f2 -d\" \"  >deseq2_gene_dw_path",extension))
}
# extension=paste0("_",contras[i],"_vs_",contras[i+1],".txt")
...
if ((grepl("hg",SPECIES)||grepl("mm",SPECIES))&&(grepl("genes",params$dtype)))
{
system(paste0("cd ",getwd(),"; python ../Scripts/FC2karyo.py DESeq2_deg_",contras[i],"_vs_",contras[i+1],".txt 2 4 ",SPECIES," > DESeq2_deg_",contras[i],"_vs_",contras[i+1],".input.karyo"))
