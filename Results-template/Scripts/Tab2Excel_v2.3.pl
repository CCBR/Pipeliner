...
#version 2.3: check for demultiplex file and allow Summary only input
...
my $summ = 0;     #print only summary output
...
my $demulti = $report->add_worksheet('Demultiplex') if (!$summ);
...
#Chipseq chart 2. column chart with 4 series
if ($pipeline == 1) {
  ...
  ##RNAseq line chart with 3 series
