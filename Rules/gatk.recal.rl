rule gatk_recal:
      ...
      threads: 32
      shell:  "{params.gatk} -T BaseRecalibrator -I {input} -R {params.genome} {params.knowns} -nct {threads} -o {output.re}; {params.gatk} -T PrintReads -R {params.genome} -I {input} -BQSR {output.re} -o {output.bam}"