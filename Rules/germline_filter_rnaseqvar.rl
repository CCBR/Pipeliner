rule germline_filter_rnaseqvar:
    ...
    shell: "{params.gatk} -T VariantFiltration -R {params.genome} --variant {input} -window 35 -cluster 3 --filterExpression \"QD < 2.0 || FS > 30.0\" --filterName \"my_filter\" -o {output.strictflagged}; {params.gatk} -T SelectVariants -R {params.genome} --variant {output.strictflagged} --excludeFiltered -o {output.strict}; module load snpEff/4.3t; java -Xmx12g -jar $SNPEFF_JAR -v -canon -c {params.effconfig} -csvStats snpeff.stats.csv -stats snpeff.stats.html {params.effgenome} {output.strict} > {output.snpeff}; mkdir -p sample_vcfs"