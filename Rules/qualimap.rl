rule qualimap:
    ...
    shell:  "module load qualimap/2.2.1; unset DISPLAY; qualimap bamqc -bam {input.bam} -c gd {params.organism} -ip -outdir {output.dir} -gff {params.regions} -outformat HTML -nt {threads} --skip-duplicated -nw 500 -p NON-STRAND-SPECIFIC"