rule canvas_wgs_germ:
    ...
    shell: "mkdir -p canvas_out; mkdir -p canvas_out/{params.sample}; cp {params.sample}.recal.bai {params.sample}.recal.bam.bai; export COMPlus_gcAllowVeryLargeObjects=1; module load Canvas/1.31; Canvas.dll Germline-WGS -b {input.bam} -o canvas_out/{params.sample} -r {params.kmer} -g {params.genome} -f {params.filter} --sample-name={params.sample} --sample-b-allele-vcf={input.vcf}"