rule make_somatic_network:
    ...
    run:
      cmd="{params.gatk} -T CombineVariants -R {params.genome} --filteredrecordsmergetype KEEP_UNCONDITIONAL --genotypemergeoption UNSORTED -o {output.vcf} --variant {params.knowns} --variant {input.vcf} --minimumN 2"
      shell(cmd)
