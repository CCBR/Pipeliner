if config['project']['annotation'] == "hg19":
  ...
  rule gatk_genotype_gvcfs_1:
      ...
      run:
        ...
        cmd="module load GATK/3.5-0; java -Xmx96g -Djava.io.tmpdir=/lscratch/$SLURM_JOBID -jar $GATK_JAR -T GenotypeGVCFs -R {params.genome} --disable_auto_index_creation_and_locking_when_reading_rods --annotation InbreedingCoeff --annotation FisherStrand --annotation QualByDepth --annotation ChromosomeCounts  --dbsnp {params.snpsites} -o {output} -L 1 -nt {threads}"+var
        print(cmd)
        shell(cmd)
