rule manta_somatic:
    input:   normal=lambda wildcards: config['project']['pairs'][wildcards.x][0]+".recal.bam",
     ...
     output: vcf=config['project']['workpath']+"/manta_out/{x}/candidateSV.vcf.gz",
     ...
     threads: 8
     shell: "mkdir -p {output.outdir}; module load manta; module load python/2.7; configManta.py --bam={input.normal} --tumorBam={input.tumor} --referenceFasta {params.genome} --exome --runDir {output.outdir}; {output.outdir}/runWorkflow.py -m local -j {threads} -g 12"