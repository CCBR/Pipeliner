...
#           vardict=config['project']['workpath']+"/vardict_out/{x}.FINAL.vcf",
    ...
    shell: "mkdir -p merged_somatic_variants; module load GATK/3.8-0; java -Xmx48g -Djava.io.tmpdir=/lscratch/$SLURM_JOBID -jar $GATK_JAR -T CombineVariants -R {params.genome} -nt 8 --filteredrecordsmergetype KEEP_IF_ANY_UNFILTERED --genotypemergeoption PRIORITIZE --rod_priority_list mutect2,mutect -o {output.mergedvcf} --variant:mutect {input.mutect} --variant:mutect2 {input.mutect2}"