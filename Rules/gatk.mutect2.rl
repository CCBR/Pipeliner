...
#            vcfFilter="mutect2_out/{x}.PASS.vcf",
    ...
    params: normalsample=lambda wildcards: config['project']['pairs'][wildcards.x][0],tumorsample=lambda wildcards: config['project']['pairs'][wildcards.x][1],gatk=config['bin'][pfamily]['GATK'],genome=config['references'][pfamily]['GENOME'],knowns=config['references'][pfamily]['MUTECTVARIANTS'],targets=config['references'][pfamily]['REFFLAT'],rname="pl:mutect2"
    threads: 4
    shell: "{params.gatk} -T MuTect2 -R {params.genome} -I:tumor {input.tumor} -I:normal {input.normal} --read_filter BadCigar {params.knowns} -L {params.targets} -nct {threads} -o {output.vcf}; {params.gatk} -T SelectVariants -R {params.genome} --variant {output.vcf} --excludeFiltered -o {output.vcfRename}; sed -i 's/NORMAL/{params.normalsample}/g' {output.vcfRename}; sed -i 's/TUMOR/{params.tumorsample}/g' {output.vcfRename}"