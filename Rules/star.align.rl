rule star_align_1:
     ...
     output: out1=temp("{x}Aligned.out.bam"),out4=temp("{x}SJ.out.tab"),out5="{x}Log.progress.out"
     params: rname='pl:star1p',prefix="{x}",starref=config['references'][pfamily]['STARREF']
     threads: 32
     run:
        import glob,json
        ...
        shell(cmd)
        # add sjdboverhand and starref back to run.json
        A=open("run.json",'r')
        a=eval(A.read())
        A.close()
        config=dict(a.items())
        config['project']['SJDBOVERHANG']=str(bestdbrl)
        config['project']["STARREF"]= config['references'][pfamily]['STARREF']+str(bestdbrl)
