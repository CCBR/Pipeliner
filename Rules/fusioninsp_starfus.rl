rule rule fusioninsp_starfus:
     ...
     shell: "module load fusioninspector/1.0.1; mkdir -p starfusion/fusioninspector/{params.sample}; FusionInspector --fusions {input.fusions} --genome_lib {params.starlib} -i {input.file1},{input.file2} --out-dir starfusion/fusioninspector/{params.sample} --out_prefix {params.sample} --prep_for_IGV"